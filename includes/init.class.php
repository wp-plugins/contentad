...
if ( ! class_exists( 'ContentAd__Includes__Init' ) ) {
	...
	class ContentAd__Includes__Init {
		...
		public static function the_content( $content ) {
			if ( is_single() ) {
				$before = ContentAd__Includes__API::get_ad_code( 'before_post_content' );
				$after = ContentAd__Includes__API::get_ad_code( 'after_post_content' );
		...
		public static function create_local_widget( $title, $adunit_id, $adunit_name, $aw_guid ) {
			contentAd_append_to_log( 'CREATING LOCAL WIDGET: ' . $title . ' ( REMOTE ID: ' . $adunit_id . ')' );
			self::update_local_widget( false, $title, $adunit_id, $adunit_name, $aw_guid );
		}

		public static function update_local_widget( $post_id = false, $title, $adunit_id, $adunit_name, $aw_guid ) {
			$new = true;
			if( $post_id ) {
				$new = false;
			...
			if ( $post_id ) {
				...
				if( ! $placement = get_post_meta( $post_id, 'placement', true ) ) {
					contentAd_append_to_log( '        POSTMETA VALUE "PLACEMENT" IS EMPTY FOR LOCAL WIDGET ('.$post_id.')' );
